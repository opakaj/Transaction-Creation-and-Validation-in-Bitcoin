...
func (T *Tx) sigHash(inputIndex int) int {
	...
	for i, tx_in := range T.txIns {
		if reflect.DeepEqual(i, inputIndex) {
			s = append(s, NewTxIn(tx_in.prevTx, tx_in.prevIndex, tx_in.scriptPubkey(T.testnet), tx_in.sequence).serialize()...)
		} else {
	...
	h := binary.BigEndian.Uint32([]byte(h256)) //bytes to int
	return int(h)
}
...
func (T *Tx) verifyInput(inputIndex int) bool {
	...
	z := T.sigHash(inputIndex)
	combined := tx_in.scriptSig + scriptPubkey
	return combined.evaluate(z)
}

func (T *Tx) signInput(input_index int, private_key *PrivateKey) bool {
	z := T.sigHash(input_index)
	der := private_key.sign(int64(z)).der()
	...
	T.txIns[input_index].scriptSig = NewScript([]interface{}{sig, sec})
	return T.verifyInput(input_index)
}
